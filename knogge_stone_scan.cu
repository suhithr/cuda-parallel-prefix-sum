...
*/
__global__ void prefixScanKernel(const uint32_t *const in, uint32_t *const out,
                                 const size_t len) {
...
void prefixScan(const uint32_t *const in, uint32_t *out, const size_t len) {
  ...
  prefixScanKernel<<<1, THREADS_PER_BLOCK,
                     THREADS_PER_BLOCK * sizeof(uint32_t)>>>(in_d, out_d, len);
