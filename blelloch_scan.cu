...
__global__ void blellochScanKernel(const uint32_t *const in,
                                   uint32_t *const block_sums_d, uint32_t *out,
                                   const uint32_t len) {
  ...
  */
  ...
  // Think of this as just building out a tree. Starting from max_threads/2
  // pairs all the way up
  for (int d = max_threads >> 1; d > 0; d >>= 1, stride <<= 1) {
    // d actually indicates how many threads run at each stage
    if (local_tid < d) {
      /* thread mapping allows us to smartly map a series of consecutive threads
      to non consecutive elements. These expressions will return elements as
...
__global__ void distributeBlockSums(const uint32_t *const block_sums_d,
                                    uint32_t *out, const uint32_t len) {
...
void blellochScan(const uint32_t *const in, uint32_t *out, const size_t len) {
  ...
  checkCudaErrors(
      cudaMemcpy(in_d, in, len * sizeof(uint32_t), cudaMemcpyHostToDevice));
  ...
  if (len <= THREADS_PER_BLOCK) {
    ...
    blellochScanKernel<<<1, THREADS_PER_BLOCK,
    ...
    checkCudaErrors(
        cudaMalloc((void **)&block_sum_d, NUM_BLOCKS * sizeof(uint32_t)));
    ...
    if (NUM_BLOCKS <= THREADS_PER_BLOCK) {
      ...
      checkCudaErrors(
          cudaMalloc((void **)&dummy_block_sums_d, sizeof(uint32_t)));
      checkCudaErrors(cudaMemset(dummy_block_sums_d, 0, sizeof(uint32_t)));
      std::cout << "Performing second blelloch scan \n";
      blellochScanKernel<<<1, THREADS_PER_BLOCK,
                           THREADS_PER_BLOCK * sizeof(uint32_t)>>>(
          block_sum_d, dummy_block_sums_d, block_sum_d, NUM_BLOCKS);
